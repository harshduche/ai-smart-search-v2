...
Based on official Qwen3-VL reranking implementation.
"""
...
class Qwen3VLReranker():
    ...
    def tokenize(self, pairs: List[Dict], **kwargs) -> Dict:
        ...
        # Apply padding using direct PyTorch operations (faster than tokenizer.pad())
        pad_token_id = self.processor.tokenizer.pad_token_id
        
        # Convert to tensors and pad to longest sequence (left padding for decoder)
        input_id_tensors = [torch.tensor(ids, dtype=torch.long) for ids in inputs['input_ids']]
        max_len = max(t.size(0) for t in input_id_tensors)
        
        # Left-pad sequences (required for left-to-right generation)
        padded_input_ids = torch.full((len(input_id_tensors), max_len), pad_token_id, dtype=torch.long)
        attention_mask = torch.zeros((len(input_id_tensors), max_len), dtype=torch.long)
        
        for i, ids in enumerate(input_id_tensors):
            seq_len = ids.size(0)
            padded_input_ids[i, -seq_len:] = ids  # Left padding
